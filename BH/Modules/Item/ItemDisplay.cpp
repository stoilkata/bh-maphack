...
{
	// Support both formats: r01-r33 (regular runes) and 01s-33s (stackable runes)
	if (code[0] == 'r')
	{
		// Regular runes: r01, r02, ..., r33
		return (BYTE)(((code[1] - '0') * 10) + code[2] - '0');
	}
	else if (code[2] == 's')
	{
		// Stackable runes: 01s, 02s, ..., 33s
...
string ItemNameLookupCache::make_cached_T(UnitItemInfo *uInfo, const string &name)
{
	...
	if (blocked)
	{
		...
		if (!has_map_action && !whitelisted)
			return new_name + " [blocked]";
	}
	return new_name;
}
...
namespace ItemDisplay
{
	...
	void InitializeItemRules()
	{
		...
		cout << "Finished initializing item rules" << endl
			 << endl;
	}
...
string ParseDescription(Action *act)
{
	...
	if (l_idx == string::npos || r_idx == string::npos || l_idx > r_idx)
		return "";
